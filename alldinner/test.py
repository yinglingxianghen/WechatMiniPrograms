...
from common.models.food.Food import Food,Person
...
def foodSearch():
    try:
        ...
        query = Person.query.filter_by(show=1)

        # if cat_id > 0:
        #     query = query.filter_by(cat_id = cat_id)

        if mix_kw:
            rule = or_(Person.name.ilike("%{0}%".format(mix_kw)), Person.tags.ilike("%{0}%".format(mix_kw)))
            query = query.filter(rule)

        food_list = query.order_by(Person.id.desc())\
        ...
        if food_list:
            for item in food_list:
                tmp_data = {
                    ...
                    'name': "%s"%( item.nicheng ),
                    'price': item.nicheng ,
                    'min_price':item.nicheng ,
                    'pic_url': UrlManager.buildImageUrl(item.main_image)
                }
                with open("foodurl.txt", "w") as f:
                    f.write(UrlManager.buildImageUrl(item.main_image))
                data_food_list.append(tmp_data)
        resp['data']['list'] = data_food_list
        resp['data']['has_more'] = 0 if len( data_food_list ) < page_size else 1
        return jsonify(resp)
    except:
        with open("food321.txt","w") as f:
