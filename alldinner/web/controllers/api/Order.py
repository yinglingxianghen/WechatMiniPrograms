...
def orderPay():
	...
	print('zzzzzzzzzzzzzzzzzzzz:',resp)
	return jsonify(resp)

@route_api.route("/order/callback", methods=["POST"])
def orderCallback():
	print('pppppppppppppppppppppppppppppost')
	result_data = {
		'return_code': 'SUCCESS',
		'return_msg': 'OK'
	}
	...
	if sign != gene_sign:
		...
		print('11111111111111sign != gene_sign:')
		return target_wechat.dict_to_xml(result_data), header
	if callback_data['result_code'] != 'SUCCESS':
		result_data['return_code'] = result_data['return_msg'] = 'FAIL'
		print("22222222222222222:callback_data['result_code'] != SUCCESS':")
		return target_wechat.dict_to_xml(result_data), header
	...
	if not pay_order_info:
		result_data['return_code'] = result_data['return_msg'] = 'FAIL'
		print("333333333333333333333333333333:not pay_order_info")
		return target_wechat.dict_to_xml(result_data), header

	if int( pay_order_info.total_price * 100  ) != int( callback_data['total_fee'] ):
		result_data['return_code'] = result_data['return_msg'] = 'FAIL'
		print("444444444444444444444444444444:int( pay_order_info.total_price * 100  ) != int( callback_data['total_fee'] ):")
		return target_wechat.dict_to_xml(result_data), header

	if pay_order_info.status == 1:
		print("5555555555555555555555555pay_order_info.status == 1:")
		return target_wechat.dict_to_xml(result_data), header
	...
	print("666666666666666666666666666success")
	return target_wechat.dict_to_xml(result_data), header
